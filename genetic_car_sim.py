...
#       super(do_stuff,self).__init__() #uncoment for framework stuff
    ...
    def mate1ptCrossover(self,parents):
        ...
        attribute_index = 0
        parents = [self.population_data[parents[0]].car_def,self.population_data[parents[1]].car_def]
        swap_point = random.randint(0,total_attributes)
        ...
        curr_parent = self.which_parent_single(attribute_index,curr_parent,swap_point)
    ...
    ''' Modified version of which_parent for one cross-over point '''
    def which_parent_single(self,index,last_parent,swp):
        if index == swp:
            if last_parent == 0:
                return 1
            else:
    ...
    def mutate(self,child):
        ...
        for i in range(child.get_wheel_count()):
            if random.uniform(0.0,1.0) > (1.0 - mutationRate):
                child.wheel_radius[i] = wheelMinRadius + random.uniform(0.0,1.0) * (wheelMaxRadius-wheelMinRadius)
        for i in range(child.get_wheel_count()):
            if random.uniform(0.0,1.0) > (1.0 - mutationRate):
                child.wheel_density[i] = wheelMinDensity + random.uniform(0.0,1.0) * (wheelMaxDensity-wheelMinDensity)
        
        if random.uniform(0.0,1.0) > (1.0 - mutationRate):
            child.set_chassis_density = chassisMinDensity + random.uniform(0.0,1.0) * (chassisMaxDensity-chassisMinDensity)
    ...
    def next_generation(self):
        ...
        stats.write("Generation: " + str(generation) + "\n")

        new_population = []
        new_population_data = []
        for i in range(n):
            stats.write("Dist " + str(i) + " " + str(self.population_data[-i-1].max_dist) + "\n")
            print("Dist " + str(i) + " " + str(self.population_data[-i-1].max_dist))
            """ Attention! Change sorting, so it takes the best three candidates,
                rather than the worst! """
            new_car = car(self.world,random=False,car_def = self.population_data[-i-1].car_def)
        ...
        while len(new_population) < self.population_size:
            ...
            self.mutate(child.car_def) # UNCOMMENT THIS WHEN YOU'VE IMPLEMENTED MUTATION
        ...
        print("new_population: " + str(len(new_population)))
        print("new_population_data: " + str(len(new_population_data)))
        print("bodies: " + str(len(self.world.bodies)))

        #change generation.
        print(len(new_population))
        print("START NEW GENERATION!!!!!!!!!" + str(generation))
